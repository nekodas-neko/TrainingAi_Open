...
describe.skipIf(!canRun)('pushMutations <-> web route parity', () => {
  ...
  it('day_checkins: the comparative answer round-trips on both paths, and NULL stays NULL (LB-124)', async () => {
    // A real column read back from a real table, because the whole risk here is a write path that
    // accepts the field and never persists it. Every layer between the body and the column is a
    ...
    await POST(jsonReq('http://localhost/api/day-checkin',
      { date: '2026-02-03', phase: 'morning', journal: 'nothing to add' }) as never)
    await repo.pushMutations(TEST_USER_ID, [{
      id: 'mut-vsy-2', domain: 'day_checkins', date: '2026-02-04',
  ...
  it('day_checkins: marking the food log complete does not clear the comparative answer (LB-124)', async () => {
    ...
    const res = await complete(jsonReq('http://localhost/api/food-logging-complete',
      { date: '2026-02-05', complete: true }) as never)
    expect(res.status).toBe(200)
