...
function setRow(over: Partial<SetHrStatsRow>): SetHrStatsRow {
  return {
    ...
    peakBpm: 165, avgBpm: 150, bpmAtEnd: 160, drop30s: 10, drop60s: 25, drop90s: 32, drop120s: 40,
...
describe('getHrRecoveryProfile chat tool', () => {
  it('returns bands and a per-band month-over-month trend', async () => {
    ...
    const band = out.bands.find((b: { label: string }) => b.label === '150–169')
    expect(band).toBeDefined()
    expect(band.n).toBe(2)
    const trendBand = out.trend.find((t: { label: string }) => t.label === '150–169')
