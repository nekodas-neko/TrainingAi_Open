...
test('the external food-database row is the shared row, and keeps its mismatch warning', async ({ page }) => {
  ...
  // BF-51 ③ put the picker's two sources behind tabs, and `Your foods` is the one it opens on — so
  // the external result is a tab away rather than below the local ones, as it used to be. Scoped to
  // the picker's own tablist: the sheet hosting it has a `My Foods` tab of its own.
