...
test('a recipe picture becomes ingredients, and the builder asks how many it serves', async ({ page }) => {
  ...
  // The ingredients land, each at its own weight.
  await expect(page.getByText('Spec Flour')).toBeVisible({ timeout: 30_000 })
  await expect(page.getByText('Spec Milk')).toBeVisible()
