...
**Version:** v1.405.2 · **Branch:** `main` · Railway auto-deploys on push to `main`.
**Last updated:** 2026-08-31.

**The DEXA correction now reaches your calorie goal, and cannot reach your raw readings (BF-2 step 3).** The correction is **+3.2 points**, derived from pairs pulled out of `dexa_scans` × `body_metrics` rather than stored — so a second scan becomes a second pair with no entry step, and there is **no new table and no migration**. It is an **offset, not a ratio**: with one pair they agree on the measured point and diverge everywhere else, and only the offset makes no claim about readings never observed. Measured live: resting burn **1832 → 1773 kcal/day**, calorie goal **1961 → 1889**, with `body_metrics.body_fat_pct` and the log sheet's own field still showing the scale's 25.3. **The safe-looking design was the wrong one.** Correcting inside the shared `listBodyMetrics` read would make a missed consumer impossible — but the Health screen seeds its body-fat log field from that read and POSTs the value back at source `manual`, which **outranks `scale_ble`**, so saving an untouched field would overwrite your raw reading and collapse the next calibration toward zero. The correction is applied per consumer instead, and `check-body-fat-correction.js` (Custom Rules, now 64) fails CI on one that forgets — **two rules, because the calorie goal never calls a deriver at all**, and the one-rule version missed it. The rule also found a consumer nobody had enumerated: the score audit, which must report what was stored. **Step 4 shipped too, so the payload now carries all three:** `bodyFat` (raw — the log sheet seeds from it), `bodyFatCorrected` (what to show), and `bodyFatIsCorrected` per reading, plus `bodyFatCalibration: { offsetPct: 3.2, pairCount: 1, source }` once per response, because you asked to be shown the offset and `pairCount` is what says how far to trust it. **`bodyFatIsCorrected` is a third field rather than a comparison** — an offset can round to zero, and "corrected by 0.0" is a different claim from "not corrected". **No screen reads any of it yet (LA-45, Lane B)**, so your Health card still shows 25.3 while your calorie goal already uses 28.5; two numbers disagreeing on screen is worse than neither being corrected, which is why that is a queue entry rather than a note ([journal](docs/overview/entries/2026-08-31-dexa-correction-consumers.md), [step 4](docs/overview/entries/2026-08-31-dexa-corrected-payload.md)).
...
**The DEXA correction exists now, and nothing reads it yet (BF-2 steps 1–2).** `body-fat-calibration.ts` + `repo.getBodyFatCalibration()` derive the correction from your 2026-08-27 scan and the same-day scale reading: **+3.2 points**, from pairs **derived** out of `dexa_scans` × `body_metrics` rather than stored, so a second scan becomes a second pair with no entry step and there is **no new table and no migration**. It is an **offset, not a ratio** — with one pair they agree on the measured point and diverge everywhere else, and only the offset makes no claim about readings never observed. **No goal, RMR or panel has moved**, deliberately: the sweep of the 22 `listBodyMetrics` call sites is step 3 and has a real design question behind it (correcting inside that read makes a missed site impossible but risks a read-then-write persisting a corrected value into the raw column). Measured against production-shaped rows: 3 of 11 readings corrected, the scan day landing exactly on the DEXA's 28.5 %, every other instrument era untouched ([journal](docs/overview/entries/2026-08-31-dexa-body-fat-calibration.md)).
