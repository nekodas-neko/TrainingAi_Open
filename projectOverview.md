...
**Version:** v1.405.1 · **Branch:** `main` · Railway auto-deploys on push to `main`.
...
**Your DEXA and RMR results have somewhere to go, and the resting rate stopped being a guess (BF-71).** The owner asked whether the calorie target used the measured RMR; it did not, and the reason was not a missing feature. `/api/measured-rmr` and `/api/dexa-scans` had shipped complete — schema, bounds, repository reads, and a `nutrition-goals/recommend` path that already consumed `getLatestMeasuredRmr` — and **nothing in the app called either one**, so both tables sat empty in production while every resting rate quoted stayed predicted. **No test could fail for that:** the routes were correct, the reads were correct, and an empty table is a valid state. `More → Health → DEXA & RMR results` is the missing half. With the owner's real 2026-08-27 figures stored, `calculateBaseline` returns **BMR 1328 / TDEE 1594** against **1485 / 1782** predicted — a **188 kcal/day** difference, which is exactly what the entry's own arithmetic forecast before the screen existed. The recommendation route moved 1950 → 1850 across the same change, but it runs an LLM and one pair of calls cannot attribute that, so the claim rests on the deterministic half. **Not device-verified:** the form hangs off `<input type="date">` and `inputMode="decimal"` fields, neither seen on the S25 ([journal](docs/overview/entries/2026-08-31-bf-71-clinical-entry.md)).
