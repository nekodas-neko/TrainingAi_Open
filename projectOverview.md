...
**Two affordances came back and the sheet that owned them is gone (LB-3, v1.347.0).** Nothing could
open `day-overlay-sheet.tsx` after Q-110, so tapping a logged exercise for its history and an activity
for its detail were dead a fortnight, unreported. Both are on `/health/day`, the NAME being the target
rather than a third icon; `health-content.tsx` lost 167 lines. The per-session HR chart was dropped.
