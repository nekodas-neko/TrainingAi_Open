// The recap week's numbers, computed once and shared by both handlers (RV-201).
//
// Everything here is deterministic and already ran before the model ever did — which is why the
// GET can serve it with no model, no rate limit and no cache, and why the week page's charts can
// paint offline. It was inline in the POST handler until RV-201; nothing about the computation
// changed in the move.

import { getRepository } from '@/lib/data'
import { formatInTimeZone } from 'date-fns-tz'
import { todayMidnightUtc, todayDayOfWeek } from '@trainingai/shared/date-utils'
...
export interface WeeklyDigestComputation {
  isoWeekKey: string
...
export async function computeWeeklyDigestMetrics(
  userId: string,
  tz: string,
): Promise<WeeklyDigestComputation> {
