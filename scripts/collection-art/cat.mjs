...
const defaultEyes = p => `
  ...
  <circle cx="47.5" cy="61" r="1.1" fill="#fff" opacity=".8"/><circle cx="75.5" cy="61" r="1.1" fill="#fff" opacity=".8"/>`
...
export function cat(p, { gearBack = '', gearFront = '', gearHead = '' } = {}) {
  ...
  <g ${o} fill="${p.light}">
    <ellipse cx="54" cy="117" rx="9" ry="6"/><ellipse cx="74" cy="117" rx="9" ry="6"/>
  </g>
  <path d="M51 115 v4 M57 115 v4 M71 115 v4 M77 115 v4" stroke="${INK}" stroke-width="1.8" stroke-linecap="round"/>
  <g ${o}>
...
export const svg = (inner, px) => `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 128 128"${px ? ` width="${px}" height="${px}"` : ''}>${RIM}<g filter="url(#cat-rim)">${inner}</g></svg>`
