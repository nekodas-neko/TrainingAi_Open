...
// to both implementer lanes is the safe failure it was designed as, and showing the same entry to
// the Orchestrator or the device agent is just noise: their work is ASSIGNED, never derived from a
// file path. With 400+ entries untagged, the dozen genuinely theirs would be unfindable.
...
if (sittingsOnly) {
  ...
  // Entries arrive in queue order, which IS priority order — so the first member of a group is its
  ...
  // Ordering by size instead (the first version) meant the biggest pile always led, which is the
  // opposite of a priority signal: nobody could say "do the back-gesture one next" without editing
  // the tool.
