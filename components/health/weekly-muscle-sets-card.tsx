...
export const WeeklyMuscleSetsCard = memo(function WeeklyMuscleSetsCard({ muscles, loading, title = "{title}" }: Props) {
    ...
    <div className="rounded-2xl bg-muted/60 border border-border p-4">
      <div className="flex items-baseline justify-between mb-3">
        ...
        <p className="text-[10px] text-muted-foreground">
          {hasTargets ? "vs your program target" : `Target: ${MIN_TARGET}–${MAX_TARGET} sets`}
        </p>
      </div>
      ...
      <div className="space-y-2">
        {muscles.map(({ muscle, sets, target }) => {
          const markSets = target ?? MIN_TARGET;
