    ...
    case 'card_nutritionDonut': {
      ...
      return (
        <div className="px-4 pb-3 relative">
          ...
          <div role="button" tabIndex={0} onClick={() => { if (!sectionEditMode) navigateWithTransition(router, pathname, "/nutrition"); }} className={cn("w-full rounded-2xl p-4 text-left active:scale-95 transition cursor-pointer", sectionEditMode && "pointer-events-none")} style={accentCardStyle(_nColor)}>
            {/* Q-415: the ring, the budget and the bar moved into their own component so the card
                reads the derived baseline the rest of the app uses instead of re-deriving one from
                the stored goal. Scalar props, so the memo on it survives this switch. */}
            <HomeNutritionCard
              metaLoading={metaLoading}
              proteinG={nutrProtein}
              carbsG={nutrCarbs}
              fatG={nutrFat}
              caloriesToday={nutrCalories}
              weeklyCalories={weekToDate?.calories ?? null}
              isWeekly={calorieType === 'weekly'}
            />
          </div>
        </div>
      )
