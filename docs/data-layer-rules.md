...
- **A nested subtree propagates a delete as an ABSENCE, not as a tombstone row** (LB-66, `program_sessions`/`session_exercises`). `getSyncDelta` re-sends a changed program's whole subtree and the client deletes every child by parent id before re-inserting, so the delta carries LIVE rows only and a tombstone simply is not in the replacement — no `deleted_at` in the local mirror, no SQLite version bump. **The load-bearing half is that every write which tombstones a child also bumps the PARENT's `updated_at`**, because that is what puts the parent in the delta at all. `saveProgram` already did; the two single-row removal paths did not, and a tombstone without that bump is strictly worse than the hard delete — invisible on the server *and* never propagated.
