...
> **Read this in `TrainingAi_Open` with one caveat:** the PR numbers below (#1375, #1376, #1379,
> #1387, #1390) are from the **private** `nekodas-neko/TrainingAI` repo, where this work was done.
> They will not resolve here. Everything they contain is already in this repo's history except
> **#1390**, which was still open at the cut — its two files (`e2e/water-log-write-path.spec.ts` and
> this handoff) were ported across by hand, so the code is here even though the PR is not.
