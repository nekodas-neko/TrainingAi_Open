...
| I28 | **The packer runs unattended now, so a refusal has no caller.** Q-541 Task 6 fires `packOuraRawBuckets` from the ingest path (per-user 6 h throttle, 8 buckets a run). A refusal is the packer declining to delete frames it could not prove were stored — the single most important signal this pipeline produces — and until Task 6 it was returned to whoever pressed the admin button | **By design:** every refused bucket is logged `[oura-autopack] REFUSED …` **and** written to `error_events` with `url='oura-autopack'`, which is what the start-of-session read covers. The run never throws: a failed pack must not turn a successful frame ingest into a 500 | `select message, count(*) from error_events where url='oura-autopack' group by 1` at session start. To stop the automatic runs without a code deploy, set `OURA_AUTOPACK=off` in Railway — nothing depends on them, the readers span both tiers, so the only consequence is that `oura_raw_samples` resumes growing at ~6.5 MB/day | None — a refusal is the *absence* of a delete. The frames stay in the hot tier exactly where they were; the blob that could not be verified stays too, so it can be examined rather than silently overwritten |
| I29 | **A complete wear-time day is overwritten by the sliver the incremental window leaves of it** (prod 2026-08-14→09-04, 22 consecutive days: `oura_daily.non_wear_time_sec` 81,000–85,500 s — 15–90 min worn — on days each carrying a 7–10 h scored night with HRV) | `wornBinsByDay` is built only from rows inside the run's window, but a wear row is written for **every** day that has a bin — including the day the floor lands mid-way through. The floor advances monotonically, so that partial write is also the LAST one the day receives: each bad day's `synced_at` is exactly the 3-day incremental margin after its own date. **Fixed (PS-30):** the day containing `rollupCutoffDs` is dropped from `wearRows` unless the cutoff falls on its local midnight — the two runs before it already wrote the day in full | Restore the 22 stale days with one **fullHistory** Redecode from `/admin/oura-ble` (the two-tier frame reader still reaches the packed archive) — owner-gated, so the fix alone leaves them wrong | None of the raw archive. The derived days read as low-wear until the Redecode, which excludes them from the HRV/RHR baselines and dims the readiness chip |
