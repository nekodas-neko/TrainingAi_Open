...
1. **RE-SCAN BEFORE CONCLUDING ANYTHING — this has now fired FOUR times.** READY went 6→9 on Lane A's BF-155; BF-110 surfaced and shipped the same hour; **BF-161 became startable minutes after I reported the lane empty**; and overnight the queue went **319 → 330** with BF-162 and BF-163 arriving at the top. **A "blocked" or "empty" finding has a shelf life of one merge.** Never report "no work" without re-running `next-item.js` first.
2. **BF-163 is NEXT and its stated Lane looks wrong — read before building.** It says `Lane: B` but names `packages/shared/src/workout/intensity-zone.ts`, and `packages/shared/**` is **Lane A** by the path rule. A change spanning both is *Lane A, engine half first*; the card half alone may be Lane B. Split it or hand the engine half over — do not edit `intensity-zone.ts`.
3. **The rest of READY is not finishable here (verified entry by entry):** BF-141, BF-135, LB-47 are **shipped work whose headings still claim a device check their bodies record as DONE** — filed as **LB-109** for the Orchestrator, which owns clearing completed entries. BF-126 needs commissioned artwork. BF-100 is a **device-only failure the harness actively contradicts** (`/more` restores 840 in Playwright, fails on the phone) — but it now has a **candidate cause and a one-tap test**, see Blocked.
4. **BF-61's fast-tap check is the keystone.** It releases **BF-94** (owner-approved, otherwise ready) and the same S25 sitting clears BF-100, BF-141, BF-135, LB-47 and most of the KEEP entries.
