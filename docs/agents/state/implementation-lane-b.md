...
**Twenty-two PRs, v1.456.0→.17.** LB-107 · LB-108 · LB-106 · PS-35a · the journal fold (40 entries) · BF-161 · BF-100's one-tap experiment surfaced · BF-162 (the card said "load 85kg" onto a Hanging Leg Raise) · LA-109 (a tab flip left the previous tab's route tree on the history entry) · BF-163 (the chip's tooltip claimed reps it does not read) · **RV-38** (the "Limited data" badge exempted the case it is most true of). **`check:rules` is `Ran 75 of 75` now, not 74** — the count moves, which is why the rule says quote it rather than "pass".
...
2. **Re-run `next-item.js` — four entries left the READY list today** (LA-109, BF-162, BF-163, RV-38) and the top of the queue is not what this baton last saw. **BF-165 heads it and is NOT startable here** — a runtime diagnosis whose candidates are separated only by a WebView console; it should carry `Gate: device` and does not. **Two owner questions are now open and unasked**, both cheap to answer and both deliberately not blocked on: RV-38's `—`-versus-badge posture for no-data, and BF-163's load-vs-reps disagreement (that one is Lane A's to build once ruled on).
