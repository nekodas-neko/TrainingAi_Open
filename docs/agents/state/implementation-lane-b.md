...
2. **⛔ A REPO-WIDE SOURCE SCAN MUST EXCLUDE THE FILE MAKING THE CLAIM.** RV-91's `Cal`/`kcal` sweep failed on **its own assertion messages**, which quote `"Cal"` four times. It passed locally because `git ls-files` does not list an UNTRACKED file, and went red on the first CI run that saw it committed. The comment above it claimed `git ls-files` made the scan safe; it made it unsafe against exactly one file.
