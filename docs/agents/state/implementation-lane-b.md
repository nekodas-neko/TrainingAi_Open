...
2. **RV-38 is next, and only HALF of it is yours.** The owner handed the *number* to Tuning on 2026-09-14, so the calibration is not either implementer lane's until signed off. What is still Lane B and unshipped: `body-battery-card.tsx:95` gates the "Limited data" badge on `battery.hasData`, so the qualification gets **weaker** as data gets worse — none at all prints `Good / Steady / 50` with no badge. Minimum fix is dropping `battery.hasData &&`. Whether no-data deserves an `—` like Readiness is the OWNER's call: ship the minimum, put the stronger posture to them, do not block on it. Also delete the stale half of the comment at lines 134–139 (it says the explainer *"only renders in the NO-DATA state"* two lines below a note saying it is always visible; the JSX is unconditional).
