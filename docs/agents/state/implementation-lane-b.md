...
- **⚠ AFTER ANY BACKLOG MERGE, DIFF THE FULL HEADING SET — counting the entries you touched is not
  enough.** PR #1481's auto-merge silently deleted RV-117 and RV-118; I checked only the three
  headings I had edited and shipped the loss. The check that catches it:
  `diff <(git show origin/main:docs/implementation-backlog.md | grep "^### " | sort) <(grep "^### "
  docs/implementation-backlog.md | sort)` — every line must be an add or remove you INTENDED.
