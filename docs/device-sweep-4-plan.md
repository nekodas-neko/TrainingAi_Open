...
and the `--sittings` list (116 checks owed elsewhere) · **Target:** web v1.465.64 (prod = `main` on 2026-09-26; updated the same day for the entries added after the first review),
...
| **I** | **RV-205, the numbers half** (probe checklist Part D) | **P24** tap-to-feedback latency on ~20 controls (flag over 100 ms). **P25** scroll smoothness on the long lists. **P26** soft keyboard vs the submit button. **P27** design-token census. **P28** motion inventory. The admin tables in P25 are read-only | read-only | 40 min |
...
| **J** | **RV-205, the picture half — P23 screen gallery** | Every tab root and pushed route, full length, warm and cold and offline; every write-free sheet open; Home, Workout, Nutrition and one sheet under gesture nav. Published to **one private Artifact** (`DV design capture — <date>`), never the repo; the URL goes in RV-205's result line | read-only | 45 min |
