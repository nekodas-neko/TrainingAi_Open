...
## Rendered, after first writing that it had not been

The first version of this entry said *"everything visual here is unverified"*. That was true of the
diff and did not have to stay true: the Playwright harness drives the real app at the 412 px dark
viewport, and a design entry is exactly what it is for. Three of four surfaces captured (Health
times out at 45 s in `next dev` and wants a longer budget).

**Confirmed on screen, not merely asserted:** the avatar reads **TU** on both Home and More, where
it read TE; lifetime volume reads **13.0 t**; Nutrition's date chevrons and settings gear render
...
**58% of the row**. That measurement is now on LB-163, so the mockup starts from an observation
...
behaviour. **RV-207's own "done when" — RV-205's P24 re-run showing a first-frame change under
100 ms on the tab bar and More rows — has NOT been run**, and needs the device. The `motion-reduce:`
branches are unexercised. Nothing here ran on the APK.
