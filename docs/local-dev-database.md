...
- **A full run can exit 1 while reporting ZERO failing tests, from a worker teardown race. It is the third distinct cause of that shape in this file — check all three before believing a red (LA-101, measured 2026-09-10).** The whole failure is one line, with every test green above it:
  ...
  ```
  A worker was torn down with a `console.*` forward still in flight. **The named file is not the culprit** — it is whichever file that worker happened to be running, and vitest says so itself (*"It doesn't mean the error was thrown inside the file itself"*). Seen twice on 2026-09-10, both during interactive work; **re-running is the correct response and both re-ran clean on identical code.**
  **What has been ruled out, so nobody re-walks it:**
  - **Not reproducible on demand.** 24 controlled full runs produced none — 8 with file-based console tracing, 8 plain, 4 with a `pnpm dev` server and 200 concurrent API requests against the same Postgres. Contention was the leading theory and it did not survive the experiment.
  - **Not the named file.** 5 solo runs of `hr-read-routes.test.ts`, clean.
