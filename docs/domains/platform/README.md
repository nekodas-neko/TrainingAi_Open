...
- [`docs/implementation-backlog.md`](../../implementation-backlog.md) **RV-143 (2026-09-23)** — `next-item.js --sittings` never consults `Gate:`, so **24 entries that are blocked ON the device agent are invisible TO it**, parked from both implementer lanes at the same time. Includes Q-51 and BF-22. The cause is a role that changed a field's meaning: before the DV role existed, `Gate: device` meant *wait for the owner*, so excluding it was right; now it means *the DV agent can unblock this*, which makes it more urgent than a `Verify:`, not less. Carries a triage of all 24 and an explicit warning against bulk-converting them to `Verify:` — that field means **shipped**.
...
- [`docs/reviews/2026-09-24-sweep-59-queue-against-code.md`](../../reviews/2026-09-24-sweep-59-queue-against-code.md) — **sweep 59, 2026-09-24: the implementer queue re-read against code, and DV-14 reproduced.** Of 59 READY entries, 5 are already shipped, 2 are wrong as written and 7 cannot be started (RV-189). Production was 22 merges behind. A local 3 GB heap cap reproduces Railway's exact failure; the changelog is not the driver, and the Sentry wrapper is. RV-188 heads Lane A.
- [`docs/reviews/2026-09-24-sweep-60-security-and-privacy.md`](../../reviews/2026-09-24-sweep-60-security-and-privacy.md) — **sweep 60, 2026-09-24: security and privacy.** Ten entries. RV-191 (HIGH): the feedback screenshot is opened as a URL in the admin panel. RV-190: admin query session state persists on the pool (reproduced locally). RV-192: unverified registration. RV-196: the ring-key plugin boundary. RV-199: three owner privacy questions. Nothing was probed on production.
- [`docs/reviews/2026-09-24-sweep-58-rules-and-performance.md`](../../reviews/2026-09-24-sweep-58-rules-and-performance.md) — **sweep 58, 2026-09-24: the unmechanised rules, and where the time goes.** RV-180 is the likely cause of DV-13: `resolveDsToMs` re-sorts 12,396 anchors per row. RV-181: one HR query is 51% of all DB time. RV-171: a failed load deletes every saved dietary restriction. RV-172: the sync pull nulls supplement dose snapshots. RV-186 is the device agent's baseline sitting.
- [`docs/reviews/2026-09-24-sweep-57-data-census.md`](../../reviews/2026-09-24-sweep-57-data-census.md) — **sweep 57, 2026-09-24: a census of the owner's production data.** Every daily series was checked for gaps, duplicates, stuck values and cross-table contradictions. Most are clean, and the clean list is recorded. Seven new entries (RV-163 to RV-169). RV-159 is answered as a body-comp re-stamp. The history-row policy is finally one owner question (RV-170).
- [`docs/reviews/2026-09-24-sweep-56-reads-nobody-ran.md`](../../reviews/2026-09-24-sweep-56-reads-nobody-ran.md) — **sweep 56, 2026-09-24: the reads nobody ran.** Of 105 entries waiting on a query or a re-measure, about 33 could be answered with one SELECT today, and 23 of those answers changed the entry. The headline: **12 of 27 recent nights are missing from `sleep_sessions`** (PS-17). Also found: an unattributed rewrite of 106 stored score rows (RV-159), and five pass tests blocked by a baseline re-derive that has never run (RV-161). RV-162 proposes a `Due:` field so an owed read fires on its date.
- [`docs/reviews/2026-09-24-sweep-55-device-verification-debt.md`](../../reviews/2026-09-24-sweep-55-device-verification-debt.md) — **sweep 55, 2026-09-24: the device checks nobody can see.** 155 Known-Issues rows say "NOT device-verified", and `next-item.js` never reads `projectOverview.md`, so DV has never seen any of them. The rows split into about 60 checks the phone alone can run (RV-155, six stations), about 55 that need the owner (RV-157, six sittings) and about 30 already answered (RV-156). `docs/device-verification-queue.md` has only 5 live rows of 41 and should retire. Production reads settled five rows outright and answered Q-270's owed read: OTS is refused `insufficient_met` on 20 of 20 days. The guard is RV-158.
...
- [`docs/reviews/2026-08-25-railway-and-db-readings.md`](../../reviews/2026-08-25-railway-and-db-readings.md) — **Railway console + production DB readings, 2026-08-25** (**Q-549's premise is falsified**: `prod_DB` reads **423 MB flat** at 0.0 vCPU, not the 0.79 GB the entry's ~$7.87/month rests on — that average spanned the 2026-08-17 `disk_full` outage, and the entry's own prediction that 0.79 GB "will return" failed on a week-warm container. Recommends closing it; `max_connections = 500` is all that is left and it is a console setting. **Q-547's deploy-marker half corroborated** — the charted window holds five known merges, so it is the opposite of the quiet baseline still owed. Plus the DB growing **~1.6 MB/day against `CLAUDE.md`'s stated 0.4**, almost all `oura_raw_samples`.) **A trap recorded rather than fallen into:** the `/api/body-battery` fault stopped **18 minutes before its fix merged**, and TN-4's guard now swallows recurrences, so a zero `error_events` count proves nothing about it.
...
- [`docs/reviews/2026-09-05-delete-reports-success-for-nothing.md`](../../reviews/2026-09-05-delete-reports-success-for-nothing.md) — **the delete surface's not-found answer, probed live with two accounts, 2026-09-05** (RV-45 — `activity-logs`'s Q-556 comment claims its 404 was made to *"match every sibling delete"*; six siblings answer `200` for a nonexistent id **and** for another account's row, with the row read back out of Postgres to prove ownership held and only the answer was wrong. RV-46 — `PATCH /api/admin/activity-types` is the one route of thirteen without the Q-463 mapper, answering 500 with an empty body and filing a correctly-refused request into `error_events`). Records **`isRetryableWriteError` as clean** — one call site, but placed in `pushMutations`'s per-mutation catch where it covers every domain. **RV-45 shipped 2026-09-05; RV-46 shipped 2026-09-06** (v1.436.10) — the thirteenth route now maps its typed error, and a source scan in `typed-error-mapper-coverage.test.ts` keeps the count at thirteen.
- [`docs/reviews/2026-09-05-body-supplied-ids-skip-the-guard.md`](../../reviews/2026-09-05-body-supplied-ids-skip-the-guard.md) — **every `PUT`/`PATCH` probed for its not-found answer, 2026-09-05** (RV-47 — `invalidUuidResponse` reaches **27 of 27** dynamic `[id]` routes and **zero** body-id ones; `admin/exercises` answers 500 for `not-a-uuid` and 404 for a well-formed missing id, `admin/users` and `meal-types` answer 500 with an empty body, and all three log the raw `22P02` statement. RV-48 — three update routes answer `200 {ok:true}` for a write that matched nothing, each against a positive control). **Clears three surfaces the baton listed as unverified**: `activity-logs/[id]/metrics` cross-user, and RV-40's `complete-workout` and `log-exercise` malformed-id behaviour. Verifies `CLAUDE.md` ownership rule (c)'s `ensureWorkoutSession` claim live. **RV-47 and RV-48 shipped 2026-09-05** (v1.436.9). Two of the five routes deliberately got no format guard — `oura_workouts.id` and `activity_types.id` are `text`, so there is no `22P02` to prevent and a uuid regex there is BF-53 repeated.
- [`docs/reviews/2026-09-15-vendor-table-rename-discriminator-audit.md`](../../reviews/2026-09-15-vendor-table-rename-discriminator-audit.md) — **Q-44 Phase 3's missing requirement, measured 2026-09-15.** The rename plan never mentions `source`, "discriminator" or "audit". **The second recording device already exists and did not share a table** — `colmi_readings` (4,237 rows), `colmi_raw_frames` (1,173), `colmi_sleep_segments` (158) are live — and the two devices use **opposite idioms**: Oura is one table per metric with a `source` column, Colmi is one table with a `kind` column. So *"use the same tables for other recording devices"* is a design problem, not a rename. `oura_bucket`, singled out as the table a second device would most need to share, holds **0 rows**. The plan's 13 tables omit **five** `oura_*` tables of 18. Recommends renaming only `oura_heartrate` (**73% Polar chest strap**, so its name actively misleads) and filing the real goal separately. **Q-44 is now `Gate: owner`.**
- [`docs/reviews/2026-09-05-app-checkpoint.md`](../../reviews/2026-09-05-app-checkpoint.md) — **the whole-app checkpoint, 2026-09-05/06** (twenty-six lanes collated; PS-24…PS-39. For this pillar: see the report's pattern sections and per-lane table). **PS-24 fixed 2026-09-06** (v1.436.11) — `auth()` refuses an inactive session on the freshly-read row, so deactivation lands on the next request; the entry stays open for the Edge middleware, which still gates on a claim it cannot verify. Its `isAdmin` sub-claim is refuted by measurement.
