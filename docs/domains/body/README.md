...
- [`docs/overview/entries/2026-09-01-feat-la-45-corrected-body-fat-display.md`](../../overview/entries/2026-09-01-feat-la-45-corrected-body-fat-display.md) — **LA-45: the screens read the DEXA correction (2026-09-01, v1.420.0).** One display rule (`components/health/body-fat-display.ts`) across Health, its metric sheet, lean mass/BMR, BMI, the day sheet, the week-day chip, Profile details and Goals. **Two invariants to not reverse:** `bodyFat` stays what the log sheet seeds from (it POSTs at `manual`, which outranks `scale_ble`), and "corrected" comes only from `bodyFatIsCorrected`, never from the two values differing. **The local seed has no DEXA scan, so the path is unreachable in the sandbox without a hand-seeded pair** — one is now in the local dev DB.
