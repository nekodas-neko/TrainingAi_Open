...
## Reference docs
- [`docs/reviews/2026-09-24-sweep-57-data-census.md`](../../reviews/2026-09-24-sweep-57-data-census.md) — **the height correction never reached stored scale composition (RV-165), 2026-09-24.** Body fat shows a false +1.1 step on 09-01, and the DEXA offset is fitted at 160 cm (+3.2 where it should be +2.2), so corrected body fat reads about a point high.
...
- [`2026-09-01-feat-la-45-corrected-body-fat-display`](../../overview/history-2026-09-10-folded-5.md#2026-09-01-feat-la-45-corrected-body-fat-display) — **LA-45: the screens read the DEXA correction (2026-09-01, v1.420.0).** One display rule (`components/health/body-fat-display.ts`) across Health, its metric sheet, lean mass/BMR, BMI, the day sheet, the week-day chip, Profile details and Goals. **Two invariants to not reverse:** `bodyFat` stays what the log sheet seeds from (it POSTs at `manual`, which outranks `scale_ble`), and "corrected" comes only from `bodyFatIsCorrected`, never from the two values differing. **The local seed has no DEXA scan, so the path is unreachable in the sandbox without a hand-seeded pair** — one is now in the local dev DB.
- **The owner’s clinical baseline** — DEXA and RMR (2026-08-27) and a 58-analyte blood panel (2026-04). **The document was removed from this repo on 2026-09-24 (RV-199) and he holds the copy**; the derived figures below stay because they are what the queued entries reason from. The measurements four queued entries were waiting on. **The first scale-calibration pair: DEXA 28.5 % body fat vs the same-day Renpho 25.3 % — the scale under-reads by 3.2 points** (BF-2 — planned 2026-08-31: the pairs are **derived** from `dexa_scans` × `body_metrics` rather than stored, and the form is an **offset** at n=1, since one pair supports neither claim). **Measured RMR 1325 kcal vs 1549 predicted, −14 %**, and Cunningham on the owner's own DEXA lean+BMC gives 1481 — **156 kcal over the measured value, so the over-estimate is not a body-composition error** (BF-33). The app's learned maintenance (1,827) lands within 5 kcal of the provider's Mild projected TDEE, implying a real activity factor of ≈1.38.
