...
- Plans: `ls docs/superpowers/plans/*sleep*` (7 today, plus archived ones under `plans/archive/`).
  ...
  [`2026-09-26-outlier-gated-rating-prompt.md`](../../superpowers/plans/2026-09-26-outlier-gated-rating-prompt.md) — ask for a sleep
  rating only on outlier nights (answers `OR-171`; built as `TN-81`/`TN-82`). Also the record of **why
  three in-sheet rating questions each decayed to zero**, which is the thing to read before adding a
  fourth.
