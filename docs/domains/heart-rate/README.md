...
- [`2026-09-02-q516-peak-bands`](../../overview/history-2026-09-10-folded-5.md#2026-09-02-q516-peak-bands) — **the HR-recovery peak bands re-cut, 2026-09-02 (Q-516).** The old `<110` boundary cut through the middle of the informative range: mean 60-second drop is **−3.5** under 90 and **5.1** at 90–104 against **12.2** at 105–119, so **42 episodes peaking 105–109 shed 11.5 bpm** and were dimmed as noise and dropped from the trend. **⛔ The entry's proposed `120+` top band was NOT shipped and must not be** — it measured `set_hr_stats` (strength, max 132) while **HRP-2 is built** and cardio cool-downs reach **168**, so collapsing the top would bucket a 168 bpm cool-down with a 120 bpm lifting rest. Shipped `<90 · 90–104 · 105–119 · 120–149 · 150+`; only the genuinely empty `170+` was removed. The module's stale *"Phase 1 seeds exclusively from set_hr_stats"* header — the line that misled the entry — is corrected, as is the §6 caveat claiming the cross-modal confound "doesn't yet bite". `informativeShare` is exposed for the honesty sentence, which is **Lane B's** and unrendered.
- [`docs/reviews/2026-09-08-walk-intensity-calibration.md`](../../reviews/2026-09-08-walk-intensity-calibration.md) — **the zone audit lives here, 2026-09-09** (TN-30, TN-32). One zone model (`ZONE_DEFS`) but **four max-HR resolvers**, three live at once and 19 bpm apart: `hrMaxFromAge` and `resolveMaxHr` give **187**, `targetAnchorMax` and `resolveBatteryHrMax` give **168**. They agree today only by coincidence — the walk's `0.70 × 116 + 52` and the Zone-2 floor's `0.60 × 135 + 52` are both 133 by unrelated arithmetic. **Owner decision: pin a 50/50 blend at 178**, replaced when the Cooper 12-Minute Run gives a measured max. ⛔ An observed maximum is a FLOOR — 2026-07-05 holds 156–168 bpm for 13 unbroken minutes, which puts the true max at 177–183.
- [`docs/reviews/2026-09-02-hr-rest-anchor-level-shift.md`](../../reviews/2026-09-02-hr-rest-anchor-level-shift.md) — **Q-515's recommended fix does not fix (a) alone — do not implement it as written, 2026-09-02.** A 90-day trailing resting anchor moves the boundary **+3.42 bpm** and the at-rest share of waking samples from **14.9% to 25.9%**, on **56 of 57 days** — the same order as the instability it fixes (26.5% → 8.2%), in the other direction, so it answers the level question the entry reserves for the owner. **The direction is structural:** during an improving trend a longer window necessarily sits above a shorter one, so no window length separates stability from level. **And the window is not doing the work:** there are only **74 days** of resting HR, so a 90-day trailing window covers **72 of them**. Freezing the anchor at a dated constant is what fixes (a) alone, at the cost of going stale silently with no cron layer to re-derive it. Entry is now `Gate: owner`. Also notes `daily_zone_minutes` stores `max_hr = 187` on all 88 rows.
- [`docs/reviews/2026-08-31-hrv-as-a-tile-metric.md`](../../reviews/2026-08-31-hrv-as-a-tile-metric.md) — **is HRV a better tile metric than HR? No — asked and answered 2026-08-31.** In contributor form against the owner's check-in, **restingHeartRate −0.491** beats **hrvBalance −0.331**, and the two correlate **+0.751 with each other (56% shared variance)** — so swapping loses a third of the correlation and buys almost nothing new. HRV is the noisiest vital (**CV 17.2%** against resting HR's 5.6%; night-to-night 7.42 ms = 13% of mean) though it is **real signal, not noise** (lag-1 autocorrelation **+0.439**). **A usable band exists** — 47–64 ms, and the stored baseline sd is **0.82×** true, unlike temperature's 12× — **but it lags, because the owner's HRV is genuinely rising** (+6.21 ms across the window while resting HR falls 2.87 bpm), so 77% of recent nights sit above it and a naive out-of-band alert fires high-side. Also resolves an apparent contradiction with BF-13 **in BF-13's favour**, and records a near-miss: the RHR baseline is fed **`rhrLowBpm`**, not `rhr_avg_bpm`.
- [`docs/reviews/2026-08-31-measured-stride-from-cadence.md`](../../reviews/2026-08-31-measured-stride-from-cadence.md) — **the owner's real stride, measured from strap cadence, 2026-08-31.** `activity_logs` already stores `distance_km` + `steps` + `cadence_spm` + `duration_min` on one row, and `segments` carries `distanceKm` + `avgCadenceSpm` per interval — so stride is directly computable. **0.739 m measured against the `0.415 × height` rule's 0.664 — the estimate is 10.1% short**, which understated every step-goal distance and calorie figure by that much. Two independent extractions agree to **0.3%** (3 sessions / 16 segments) and `cadence × duration` reproduces recorded steps to **+0.13%**. **⛔ But a single constant is still wrong: stride vs pace is r = −0.885**, 0.83 m at 10:00/km against 0.62 m at 14:00/km, and the tracked walk is only **27–94% of a day's steps** (median ~48%) — the rest sits at an unknown slower stride that nothing stored can measure. Also finds a **corrupt `activity_logs` row (2026-07-01: 4,970 steps in 0.2 min, more than the whole day's 1,358)**, so any derivation needs a sanity gate.
- [`docs/reviews/2026-08-26-hr-tile-and-activity-pacing.md`](../../reviews/2026-08-26-hr-tile-and-activity-pacing.md) — **what the HR tile should actually show, 2026-08-26.** The owner's two alternatives were both tested and **neither choice is the lever**: against `perceived_recovery` (1 = fully recovered … 5 = wrecked, so positive r is correct), waking-rest HR reads **+0.176 raw / +0.291 baseline-relative** and nightly resting HR **+0.129 / +0.278**. **Expressing either as a delta from the owner's own baseline roughly doubles it; picking between them barely moves it** — so the defect is showing an absolute bpm at all. Also reconciles the pillar review's **+0.557** against this **+0.129**: that figure was the baseline-relative *contributor score*, measuring **−0.553 (n=35)** here, the sign carried by two scales running opposite ways — and **dropping the 4 `provisional: true` days is what takes it from −0.395 to −0.553**. A **waking-rest HR** (10th pct 08–21, 70 days, 984 samples/day, moving 6.24 bpm/night) is a real second-tile candidate and the better stress proxy, but **nothing in the app computes it**.
- [`docs/reviews/2026-08-26-pillar-review.md`](../../reviews/2026-08-26-pillar-review.md) — **the five Home pillars answered, 2026-08-26.** The HR tile's "52" is the **7-day average** resting HR (`oura-score-chip-row.tsx:390` → `readiness.restingHr`). Measured over 50 nights: the nightly value moves **2.11 bpm** night to night, the tile's average **0.33** — **it discards 84% of the movement**, in the signal that is the *strongest* predictor of the owner's own check-in (r = **+0.557**, best of nine). Filed **TN-13**: show last night's value with its delta against baseline. **Do not swap in HRV** — more responsive, less correlated with felt state (+0.427). **✅ TN-13 shipped 2026-08-30** ([journal](../../overview/history-2026-09-10-folded-3.md#2026-08-30-feat-hr-tile-nightly-resting)) — the tile reads `restingHrLastNight` and renders `50 · −7 vs usual`. Re-measured over **71** nights on the day it shipped: 2.50 / 0.58, i.e. **77 %** discarded rather than 84 % — same conclusion, and the figures above are the 50-night originals. **Still owed: the S25 check**, because the cue text grew from one word to five across 20 layout styles.
