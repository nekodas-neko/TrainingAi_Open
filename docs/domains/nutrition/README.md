...
- [`docs/reviews/2026-09-03-coach-write-bounds-vs-user-routes.md`](../../reviews/2026-09-03-coach-write-bounds-vs-user-routes.md) — **the Coach's four undriven handlers, and its number bounds, 2026-09-03** (sweep 44, RV-41). Closes the gap the 2026-08-18 Coach review named in its own method section — `nutrition_targets`, `user_goals`, `injury`, `program_phase` and `early_deload` were *"read, not driven"*. **All four are clean**: they write correctly, undo correctly (a created injury is soft-deleted so the sync delta carries a tombstone), refuse another account's target with 404, and refuse a stale proposal with 409 plus a drift array naming field, expected and actual — which materially strengthens **Q-467**, since the Undo button it asks for has working machinery behind it. **RV-41** is the finding: the patch schema bounds every goal number at `max(100_000)` under a comment claiming it refuses 26,000 kcal, and it does not — one column, two validators, and the Coach's is looser on six of seven fields (protein/carbs/fat by **50×**). `stepsGoal` also loses its `.int()`, turning a client input error into a 500.
...
- [`docs/reviews/2026-09-05-delete-reports-success-for-nothing.md`](../../reviews/2026-09-05-delete-reports-success-for-nothing.md) — **the nutrition delete routes' not-found answer, 2026-09-05** (RV-45 — `food-logs/[id]`, `saved-meals/[id]`, `meal-types/[id]` and both supplement deletes answer `200` for a row that does not exist and for another account's, while `meal-plans/[id]` in the same tree answers 404. Finds the boundary of the 2026-08-18 review's decision not to file these: its idempotency argument holds for the owner's own already-deleted row and fails cross-account, where the desired end state is *not* "row absent").
- [`docs/reviews/2026-09-05-body-supplied-ids-skip-the-guard.md`](../../reviews/2026-09-05-body-supplied-ids-skip-the-guard.md) — **the nutrition update surface, 2026-09-05** (all dynamic `PUT`/`PATCH` routes answer 404 for a ghost id — sweep 47's finding is a `DELETE` finding. RV-47/RV-48 land on the collection `PATCH /api/nutrition/meal-types`, which 500s with an empty body on a malformed id and answers `200 {ok:true}` for a reorder that moved nothing). Records **`PUT /api/nutrition/saved-meals/[id]` as correct**: creating a row at a client-supplied UUID is `writeSavedMeal`'s deliberate upsert with `setWhere: eq(userId)`, and it refuses the second account's id with the row unchanged. **RV-47 and RV-48 shipped 2026-09-05** (v1.436.9): the reorder now guards every `orderedIds` entry and applies the whole order or none of it.
