...
- [`docs/reviews/2026-09-03-acwr-ewma-day-shift.md`](../../reviews/2026-09-03-acwr-ewma-day-shift.md) — **what the uncoupled-EWMA ACWR switch would actually change, measured before writing it, 2026-09-03** (Q-279). Over 95 comparable days of real history the **scales agree** (mean 0.919 → 0.955), so **no threshold has to move with the switch** — the result that keeps this a small change rather than a recalibration of `lowMax`, `EARLY_DELOAD_ACWR_MIN` and `ACWR_TAPER_START` together. But ~20% of days flip at the early-deload boundary (12/95 → 15/95 firing, **19 individual days change**), and the over-exertion taper goes from **4 firings to 1**, because the EWMA's weighting damps exactly the single-heavy-session spike the taper reacts to — the sharpest question in the document and the owner's to answer. Also refines the entry: chronic divides by the **observed span in weeks**, not a flat 4, so there is no `/ 4` to go looking for. Entry is now `Gate: owner`.
...
- [`2026-09-20-feat-or118-movement-balance-card`](../../overview/history-2026-09-22-folded-1.md#2026-09-20-feat-or118-movement-balance-card)
...
- [`2026-09-09-fix-injury-header-crowding`](../../overview/history-2026-09-12-folded-1.md#2026-09-09-fix-injury-header-crowding)
  ...
  branch**, so two stacked banners pushed set 1 under the logging sheet with nothing to recover it.
  The full banner is now on the ready screen — which carried **no injury warning at all** before
  this, so it used to arrive after the weight was chosen — and during the set it is a chip with Swap
  intact. The AMRAP banner is **deleted**: the ready screen already says the same thing at more
...
- [`docs/reviews/2026-08-18-rpe-autoregulation-calibration.md`](../../reviews/2026-08-18-rpe-autoregulation-calibration.md) — **RPE autoregulation calibrated, 2026-08-18** (Q-514 — `RPE_DEAD_BAND = 1.5` is correctly placed and must not move; the bias is in the input. `expectedRpe`'s **floor** clamp binds on 6.5% of sets — ordinary 50–67% × 7–13-rep accessory work, not warm-ups — giving them a **+1.89** mean delta against **−0.34** for everything else. Excluding them removes **64% of back-off triggers and zero push triggers**, so two thirds of the engine's 5–10% load cuts were a clamp artefact. **✅ SHIPPED 2026-09-02** — `isExpectedRpeRepresentable` beside `expectedRpe`, and `perExerciseRpeDelta` drops those sets rather than neutralising them; `rpeTrendFromSets` deliberately still sees every set, because it is the emergency-deload net and the bias makes it fire early. The re-measure is Tuning's. Also: `calcAmrap1RM`/`amrapScaleFactor` have **no production call site**).
