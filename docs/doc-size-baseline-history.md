...
## 2026-09-05 — `docs/implementation-backlog.md` → 17981, `projectOverview.md` → 9903 (sweep 48: RV-47, RV-48)
...
## 2026-09-08 — `docs/implementation-backlog.md` → 18702 (LA-66 shipped)
...
is where it will be read. Re-derived from the merged file each time `main` moved under this branch — LA-60 (#942), then
#944 — which is the number changing because the queue did, not because this entry did.
...
## 2026-09-08 — `docs/implementation-backlog.md` → 18763 (LB-63 shipped)
...
Q-112e's split (+42), then PS-39 three times over (+2, −3, +6) — and the recompute happened four
times, once per rebase, on a PR whose whole diff is one `className`. That is the LB-52 shape in
miniature: this file changes length on essentially every merge, so its baseline conflicts with
...
## 2026-09-09 — `docs/implementation-backlog.md` 19,873 → 19,882 (+9), BF-51 ③ shipped

The entry stays: ① and ② are still held, so only ③'s line changes, and it grows because two of the
three lines it becomes are decisions rather than status.

The load-bearing one is the naming. The owner said *"a tab like the other place"*, and copying that
...
**Amended on rebase:** the entries ceiling in this branch read 343 from the +1-per-PR treadmill;
#1052 raised it to **360** by owner decision while this was in flight, and that number is taken
instead. The treadmill is over — LB-94 (re-pointing the ~300 citations at the batched history) is
the real fix, and the next session to reach 360 does that work rather than moving the number again.
The backlog baseline is re-derived from the merged file (20,037) rather than spliced from either
side, since both branches added entries.
