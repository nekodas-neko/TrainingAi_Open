...
**locally, on the owner's machine with the S25 on USB**, and is the only role that can see the real
app in the real APK — it drives it over the DevTools protocol (`scripts/device/**`) and answers the
104 checks the device gate has accumulated. Those five end at a docs-only PR and never write product
code — which is what keeps the collision surface to Lane A against Lane B. (Device Verification owns
`scripts/device/**`, its own harness, because whoever first points it at a phone is the only session
that can fix it.)
...
- **Entry IDs come from your own letter and count up forever — there is no band and no pointer.**
  ...
  Device Verification `DV-` · one-off sessions `PS-`. Find
...
- **A device check answers VERIFIED / FAILED / COULD NOT CHECK, and never a fourth thing.** A
  **FAILED is work, not verification debt** — it goes back to the lane that owns the surface with
  ...
  result: three-button navigation alone reports every safe-area inset as `0`, which makes a broken
