...
- **Everything reaches `main` through a pull request — direct pushes to `main` are blocked.** A PR is the only path, and `main` blocks force-pushes and deletions. **⚠ But the required checks (Lint, Tests, Build, Custom Rules, Migration Check — type checking rides inside Build, it isn't a separate job) are NOT ENFORCED AT MERGE, and this line used to say they were (LB-134, verified 2026-09-24).** Measured on PR #1467: `merge_pull_request` returned success at **10:18:18Z** with the `Tests` job still running; it reported **failure** at 10:18:29Z, eleven seconds later, and `main` took a red commit. The mechanism is that `main` is protected by a **Ruleset** with no classic branch-protection rule beside it — the same gap that makes `enable_pr_auto_merge` answer *"Protected branch rules not configured for this branch"*, re-confirmed 09:15 UTC that day. **So a merge that succeeds is NOT evidence the checks passed. Read the job conclusions yourself before merging** (see the CI/CD section). Turning the checks on is `LB-52`/`Q-297`, the `owner-branch-protection` batch, and it is the owner's. Always develop on a feature branch, open a PR, let CI run, then squash-merge. (Merges use the GitHub MCP tools, since the sandbox git proxy can't push to `main` either.)
