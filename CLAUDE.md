...
- **Also at session start, read `error_events` in production** — it is the only view of faults that never reach a human. **It does NOT prune, despite what this line said until 2026-09-01 and what `lib/export/export-map.ts` still says** — there is no `DELETE`, no `pg_cron` and no trigger, and the oldest row measured 32 days old against the claimed 30-day retention (BF-93). Read it anyway, and read it early: a fault that stops on its own still goes unnoticed, which is the real reason for this rule. What changes is the consequence of *not* reading — the record waits rather than expiring, and the table is now the second-largest object in the database at 52 MB. The first read of that table (2026-08-04) found three faults, **two of which had already stopped before anyone looked**. One query via the admin endpoint:
  ```
