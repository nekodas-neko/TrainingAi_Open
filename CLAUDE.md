...
- **Everything reaches `main` through a pull request, and since 2026-09-25 that is ENFORCED rather than merely documented.** The `ProtectMain` ruleset is **Active** and requires five checks — **Lint, Tests, Build, Migration Check, Custom Rules** (type checking rides inside Build; **E2E is deliberately NOT required**, owner's call, pending `LB-56`). Also enforced: a PR before merging, **squash the only permitted merge method**, force-pushes blocked, deletions blocked. Approvals are **0** — every merge here is the owner or an agent, so requiring one would mean nothing ever merges. The bypass list is **empty**.
  **⚠ THIS PARAGRAPH SAID THE OPPOSITE UNTIL 2026-09-25, AND BOTH VERSIONS WERE WRONG ABOUT THE MECHANISM (OR-164).** It first claimed the checks were enforced; `LB-134` corrected that to *not* enforced and blamed *"a Ruleset with no classic branch-protection rule beside it"*. The real cause was simpler and nobody looked: **the ruleset's Enforcement status was `Disabled`** — created 2026-08-17 with all six checks already listed, and switched off for five and a half weeks. So the rules were configured and inert.
  **What that means for anything you read about this period.** A merge before 2026-09-25 proves nothing about its checks — PR #1467 merged at **10:18:18Z** with `Tests` still running and reported **failure** eleven seconds later, and `main` took a red commit. **And the force-push/deletion guarantees this file asserted were also false for those weeks**, because the `deletion` and `non_fast_forward` rules were sitting in the same disabled ruleset. They are real now.
  **Still verify before merging.** `get_check_runs` on the PR — every job `completed` AND `success` — is a habit worth keeping even though the merge button now enforces it; it is what tells you *why* a merge is refused. `pnpm check:rules` is the Custom Rules job ONLY, not the gate; run `pnpm test` too. Always develop on a feature branch, open a PR, let CI run, then squash-merge. (Merges use the GitHub MCP tools, since the sandbox git proxy can't push to `main` either.)
  **Two consequences to expect.** A flaky job now BLOCKS a merge instead of slipping through, and **if CI itself breaks there is no path to push the fix** — the escape hatch is adding the owner to the ruleset's bypass list temporarily, which is deliberately not pre-configured. **`strict` (require branches up to date) is OFF on purpose:** `main` takes a commit roughly every 8 minutes against a ~7-minute CI run, so requiring a current base can livelock — one docs PR needed seven re-merges on 2026-09-24 without it. Revisit only once `enable_pr_auto_merge` is confirmed working, which it may now be.
...
- **`Lane:` is how any agent hands work to any other — it is the channel, not a label.** `A`/`B`
  ...
  round of thinking first; `DV` is work whose deliverable needs the phone; **`T` (2026-09-26, OR-178)
  is a scoring change owing a Tuning PROPOSAL before anyone may build it** — fifteen sat on a
  placeholder `Needs:` for want of a field saying so; a `T` entry names its implementation lane in
  prose, and Tuning re-lanes it once the proposal lands. Review can put something
...
**Three CI-monitoring gotchas that cost a session each:** (1) a bash `curl` to `api.github.com` using `$GITHUB_TOKEN` is **not authenticated** in this environment — it's a non-authenticating proxy placeholder, and reading its silence/failure as "still running" or "still green" is wrong. Always use the GitHub MCP tools (`get_check_run`, `get_job_logs`, etc.), never bash `curl`, to check CI/PR state. (2) A check run that fails in 2–3 seconds with **no logs at all** is a transient GitHub Actions startup blip, not a real failure — re-trigger it with an empty commit rather than diagnosing a failure that has no evidence. (3) **`get_check_runs` can lag reality by 30+ minutes, and a job's logs 404 for exactly as long — so "still in progress" is not evidence that anything is stuck.** Measured 2026-08-17 (PR #20): `Tests` read `in_progress` for ~25 min after it had completed at 06:39:36, and `Lint` — a 43-second job — read `in_progress` for ~35 min. Log fetches returned HTTP 404 throughout, which looks like confirmation of a running job and is not. Two hours went into diagnosing a stall that never existed, including one pointless empty-commit re-trigger. **⛔ THE MERGE BUTTON IS A GREEN CHECK AGAIN — as of 2026-09-25, and this passage has now been wrong in BOTH directions (OR-164).** It first claimed the merge validated against branch protection; `LB-134` proved otherwise on PR #1467, which merged at **10:18:18Z** with `Tests` still running and took a red commit. The cause was not *"a Ruleset the API cannot see"* as recorded — **the ruleset's Enforcement status was `Disabled`**, and the owner switched it to Active on 2026-09-25. Five checks are now genuinely required. **Do not read the old warning as still current, and do not read this as licence to stop checking:** a merge being refused tells you nothing about WHY, and `get_check_runs` does. **Read the job conclusions: every job `completed` AND `success`.** `get_job_logs` with `failed_only: true` is the cheap complement — an empty list means nothing failed — but it reports 0 while jobs are still running, so it is not sufficient alone; that misread cost a merge attempt on 2026-09-24. Never use `list_workflow_jobs` for this; it floods context. Note the RUN can stay `in_progress` past the required jobs because E2E is advisory and not required; that is not a failure. When the checks endpoint looks frozen, use that read rather than trying the merge to find out. To tell a slow job from a stale read, compare **job-level `started_at`/`completed_at` per step** from `list_workflow_jobs` on a recent run, never the run's `updated_at` (which is itself stale — it read 9 seconds after start on a run that had been going an hour).
...
- **`freshWithinTtl: true` requires a written invalidation proof**: list every write that changes the payload and show each one's group contains the key. **And before writing that proof, check the payload is a PURE FUNCTION OF STORED ROWS — if it is not, no proof exists and the key is disqualified** (RV-67, 2026-09-25, where three of five candidates failed here for three different reasons). A payload that is DERIVED makes every input's writer a writer of this key; one that depends on `today` serves yesterday's answer across midnight; and one that decays with the clock — `muscle-recovery` recomputes from `Date.now()` — changes with **no writer at all**, so there is nothing an invalidation group could ever be made to catch. A key being `TTL_LONG` and already in a group says nothing about any of this, which is exactly how that candidate list was drawn up. A today-scoped key is disqualified for a fourth reason: `cachedFetchToday` has no such parameter. A missed writer converts a stale flash into hours of hard staleness. A today-guard on the cache seed isn't enough on its own — the `cachedFetch` onData hit path needs the same guard, or use `cachedFetchToday`.
...
- **On `docs/implementation-backlog.md`, a conflict is almost always TWO DELETIONS — keep neither side.** Each PR removes the entry it finished, so when two land together the markers wrap *different completed entries*, and "keep both" restores both. That is not hypothetical, and **it has now happened three times**: it silently put **LB-4, Q-454, Q-455 and Q-465 back into the queue after they shipped** on 2026-08-23, they were removed in #348 the same day — and **LB-4 came back again four commits later in #349**, from a branch cut before #348 landed. A rule cannot reach a branch that predates it, which is the argument for the check rather than against the rule. The inverse holds on append-only files (`known-issues-resolved.md`, `doc-size-baseline-history.md`), where a conflict is two *additions* and keeping both is right. **The doc-size baselines no longer conflict at all** — LA-33 split the shared map into one `docs/doc-size/<path>.size` file per tracked doc, so two PRs raising two different documents touch no common line; two raising the *same* document still conflict, which is correct, because they genuinely disagree about one number. **The REASON for a raise now goes in its own file too** (LB-130, 2026-09-26): `docs/doc-size/history/YYYY-MM-DD-<branch-slug>.md`, never prepended to the 18,000-line `docs/doc-size-baseline-history.md` — that was the second of the three files colliding on every pair of concurrent PRs. The batched file keeps the older notes and a compaction sweep folds new ones in, as for `docs/overview/entries/`. Nothing parses either, so a missing note fails no check; hence written down rather than enforced. **The two cases look identical from the marker alone — read the headings before choosing.**
