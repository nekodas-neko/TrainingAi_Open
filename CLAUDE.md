...
- **At the start of every session, work out which standing agent you are** — read [`docs/agents/README.md`](docs/agents/README.md). Four roles run against this repo (Implementation in two lanes, BugFix, Tuning, Review), up to five sessions concurrently, and that file is the contract between them: who owns which files, which letter your entry IDs come from, what you may merge without asking, and how you hand your role to a successor. If you were started from one of the prompts in `docs/agents/prompts/`, read your own baton at `docs/agents/state/<agent>.md` before anything else — it is the state your predecessor left you.
...
- **On `docs/implementation-backlog.md`, a conflict is almost always TWO DELETIONS — keep neither side.** Each PR removes the entry it finished, so when two land together the markers wrap *different completed entries*, and "keep both" restores both. That is not hypothetical: it silently put **LB-4, Q-454, Q-455 and Q-465 back into the queue after they shipped** on 2026-08-23, and nothing caught it — `check-backlog-pointers` only fails on a heading that says ✅/SHIPPED, and a resurrected entry reads exactly like an open one. The inverse holds on append-only files (`known-issues-resolved.md`, `doc-size-baseline-history.md`), where a conflict is two *additions* and keeping both is right. **The two cases look identical from the marker alone — read the headings before choosing.** A tempting checker does not work: 25 ids sit in both a queue heading and a journal title today, and most are correct (an entry that shipped half its work stays queued with a `Keep:` line).
