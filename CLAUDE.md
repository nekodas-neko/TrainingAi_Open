...
- **Also at session start, read the database size** — one query, same endpoint, beside the `error_events` read:
  ...
  **Baseline: 171 MB total on 2026-08-18**, after the packing work took `oura_raw_samples` from 563 MB to
  50 MB. Growth was stated as ~0.4 MB/day; **measured 2026-09-18 it is 1.71 MB/day against a 224 MB total** (RV sweep 50). That number is wrong and the conclusion it supports is not — the growth is bounded and explained: `oura_raw_samples` holds ~8 days under a 31-day cap because the packer reclaims into `oura_raw_packed`, `oura_heartrate` spans 88 days against its ~90-day window (steady state), and `rr_intervals` spans 60. **Compare against ~1.7 MB/day, and treat a departure from the SHAPE above — a window that stops reclaiming — as the signal, rather than the daily figure alone.** Anything materially above that trend gets a Known-Issues row the
  same session.
