...
export function isPlausibleHrSample(bpm: number, recent: number[], maxJump = 30): boolean {
  ...
  // No baseline is not a reason to reject a physiologically-plausible reading. Unreachable given
  // the length guard above, but stating it beats a non-null assertion that hides the question.
  if (baseline === null) return true
  return Math.abs(bpm - baseline) <= maxJump
}
...
export function rollingMedian(values: number[], window = 5): number[] {
  ...
  return values.map((v, i) => {
    ...
    // Every slice contains at least `values[i]`, so the null branch cannot be taken.
    return median(slice) ?? v
  })
}
